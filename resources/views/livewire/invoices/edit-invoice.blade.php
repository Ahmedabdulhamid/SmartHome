<form wire:submit.prevent="updateInvoice">
    ...
    <div class="row">
        <div class="col-md-6 mb-3">
            <label for="edit_client_id" class="form-label">العميل</label>
            <select class="form-control @error('client_id') is-invalid @enderror"
                    id="edit_client_id" wire:model="client_id">
        ...
        <div class="col-md-3 mb-3">
            <label for="edit_invoice_date" class="form-label">تاريخ الفاتورة</label>
            <input type="date" class="form-control @error('invoice_date') is-invalid @enderror"
                   id="edit_invoice_date" wire:model="invoice_date">
        ...
        <div class="col-md-3 mb-3">
            ...
            <input type="date" class="form-control @error('due_date') is-invalid @enderror"
                   id="edit_due_date" wire:model="due_date">
            @error('due_date')<div class="text-danger">{{ $message }}</div>@enderror
        </div>
    </div>

    <div class="mb-3">
        <label for="edit_invoice_number" class="form-label">رقم الفاتورة</label>
        <input type="text" class="form-control @error('invoice_number') is-invalid @enderror"
               id="edit_invoice_number" wire:model.blur="invoice_number">
                    ...
                    <tr>
                        <td>
                            <input type="text"
                                ...
                                wire:model.blur="items.{{ $index }}.description" placeholder="وصف المنتج/الخدمة">
                            @error('items.' . $index . '.description')<div class="text-danger small">{{ $message }}</div>@enderror
                        </td>
                        <td>
                        ...
                        <td class="text-center">
                            <button type="button" class="btn btn-danger btn-sm" wire:click="removeItem({{ $index }})">X</button>
                        </td>
                    </tr>
                @endforeach
            </tbody>
    ...
    <div class="modal-footer px-0 pb-0 mt-4">
        ...
        <button type="submit" class="btn btn-primary">
            <span wire:loading.remove wire:target="updateInvoice">حفظ التعديلات</span>
            <span wire:loading wire:target="updateInvoice">جاري التحديث...</span>
        </button>
    </div>
</form>
